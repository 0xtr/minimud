int32_t print_player_speech (const int32_t socket_num, const uint8_t *say) {
    ...
    strncpy((char*)buffer, get_player_pname(pnum), strlen((char*)get_player_pname(pnum)));
    strncat((char*)buffer, " says: ", 7);
    strncat((char*)buffer, &say[TOKEN_SAY_CMD_LEN], BUFFER_LENGTH - TOKEN_SAY_CMD_LEN);
