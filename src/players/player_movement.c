...
int32_t move_player(const int32_t pnum, const int32_t DIRECTION)
{
	int32_t rv = 0;
	...
	rv = lookup_room_exits(pnum, xadj, yadj, zadj);

	if (rv == -1) {
		rv = print_to_player(pnum, INVALDIR);
		return -1;
	...
	rv = print_to_player(pnum, DIRECTION);
	if (rv == 2) {
		print_to_player(pnum, SHOWROOM);
		return adjust_player_location(pnum, x, y, z);
	} else if (rv == 0) {
		print_to_player(pnum, SHOWROOM);
		return adjust_player_location(pnum, -1, -1, -1);
	} else {
		return EXIT_FAILURE;
	}
}
